...
public class MainServer {
	...
	public static void main(String[] args){
		...
		MainServerCmdLineValues serverValues = new MainServerCmdLineValues();
		...
		try {
			...
			port = serverValues.getMainServerClientPort();
			coordination = serverValues.getMainServerServerPort();
			
			System.out.println("main server port :" + port);
			System.out.println("main server coordination port :" + coordination);

			new MainServerServerConnection().start();
			new MainServerClientConnection().start();
			
		} catch (CmdLineException e) {
			System.out.println("using parameter -cp <clientport> -sp <serverport>");
			e.printStackTrace();
		}
	...
	public synchronized void RemoveServerAnnouncement(SSLSocket serverSocket,Conf serverConf){
		...
		for(int i=0;i<server_connection.size();i++){
			try{
				...
				SSLSocket socket = (SSLSocket) sslsocketfactory.createSocket(server_state.get(i).getServerAddress(),server_state.get(i).getCoordinationPort());
				BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(socket.getOutputStream(), "UTF-8"));
				JSONObject notifyremainserver = ServerMessage.notifyOneServerDown(serverConf,room_name);
